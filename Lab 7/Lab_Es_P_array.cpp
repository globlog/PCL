...
void loop_main(long int n) {
	long int l,w;
		std::pair<long int,long int> * sprinklers = new std::pair<long int,long int>[n];
		long int x,r,a;
		std::cin >> l >> w;
		...
		std::partial_sort( sprinklers, sprinklers+c,sprinklers+n, left_is_less_ll);
		...
		if (some){
			if (left_is_less_rr(final,best_sprinker) && (best_sprinker.first != l+1) ) {
				sprink_count+=1;
				std::cout << sprink_count << std::endl;
			} else {
				std::cout << -1 << std::endl;
